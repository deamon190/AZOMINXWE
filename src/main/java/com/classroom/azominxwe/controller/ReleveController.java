...
public class ReleveController {
    ...
    public void downloadReleve(@PathVariable Long id, HttpServletResponse response, HttpServletRequest request, Model model) throws IOException {
        ...
        List<Note> notes = noteRepository.findByEleveAndTrimestre(eleve, trimestreActif);
        ...
        Map<Long, Note> notesMap = notes.stream().collect(Collectors.toMap(note -> note.getClasseMatiere().getClasseMatiereId(), note -> note));
    ...
    private String renderHtml(Model model, String imageUrl) {
        ...
        Map<Long, Note> notesMap = (Map<Long, Note>) model.getAttribute("notesMap");
        ...
        html.append("<h1>Relevé de Notes</h1>")
                ...
                .append("<p><b>Nom:</b> ").append(eleve.getNom()).append("</p>")
                .append("<p><b>Prénom:</b> ").append(eleve.getPrenom()).append("</p>")
                .append("<p><b>Trimestre:</b> ").append(nomTrimestre).append("</p>");

        html.append("<table><tr>")
                .append("<th>N°</th><th>Matière</th><th>Note</th><th>Coefficient</th><th>Note Coefficientée</th>")
                .append("</tr>");
        Integer i=1;
        for (ClasseMatiere cm : classeMatieres) {
            Note note = notesMap.get(cm.getClasseMatiereId());
            double noteValue = note != null && note.getNote() != null ? note.getNote() : 0.0;
            Integer coef = cm.getCoefficient();
            double noteCoef = noteValue * coef;

            html.append("<tr>")
                    .append("<td>").append(i).append("</td>")
                    .append("<td>").append(cm.getMatiere().getNomMatiere()).append("</td>")
                    .append("<td>").append(noteValue).append("</td>")
        ...
        if (Objects.equals(nomTrimestre, "Trimestre 3")) {
            ...
            html.append("<p>Moyenne Annuelle: <b>").append(moyenneAnnuelle).append("</b></p>");
        }
